...
def register_default_algorithms() -> None:
    ...
    # GRACE variants (feat/grace-critic): base x Grace strategy x routed
    # serving head. Five-keys (requires_confounder_u=False — the CBN fit and
    # router never read the realized U) and episode-grouped (the episode-
    # latent EM + router statistics need whole episodes). The CLASSICAL algo
    # variants default to the U=0 REFERENCE-STRATUM deploy (the return-correct
    # deploy under the action-gated confounder — the same cell-9 semantics as
    # offline_dqn_proximal_action); the ablation arms deploy the marginal
    # (spec-level, oracle-comparable). Same discrete bases; a continuous
    # *_grace is a clean KeyError.
    ...
    def _grace_u0(builder):
        """Classical-variant wrapper: default deploy='u0' (overridable via an
        explicit grace_options)."""

        def _wrapped(**kwargs):
            gopts = {"deploy": "u0", **(kwargs.get("grace_options") or {})}
            return builder(**{**kwargs, "grace_options": gopts})

        return _wrapped

    for _gname, _gbuilder in (
        ("offline_dqn_grace", build_grace_dqn),
        ("bcq_grace", build_grace_bcq),
        ("cql_grace", build_grace_cql),
        ("iql_grace", build_grace_iql),
    ):
        registry.register(
            _gname,
            AlgorithmSpec(
                builder=_offpolicy_recurrent_guard(_gname, _grace_u0(_gbuilder)),
                kind="off_policy",
                data_regime="offline",
                needs_episode_grouping=True,
            ),
    ...
    registry.register(
        "offline_dqn_recurrent_grace",
        AlgorithmSpec(
            # WITHOUT the recurrent guard (accepts critic_network=lstm), like
            # the other recurrent strategy rows. DQN base only.
            builder=_grace_u0(build_grace_dqn_recurrent),
            kind="off_policy",
            data_regime="offline",
            needs_episode_grouping=True,
        ),
    ...
    registry.register(
        "online_dqn_grace",
        AlgorithmSpec(
            # The online mutilated-channel arm: refresh-cadence CBN refits from
            # the rolling episode buffer (Gate-B label persistence).
            builder=_offpolicy_recurrent_guard(
                "online_dqn_grace", _grace_u0(build_grace_online_dqn)
            ),
            kind="off_policy",
            data_regime="online",
            needs_episode_grouping=True,
