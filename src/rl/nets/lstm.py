...
class LSTM(nn.Module):
    """Recurrent trunk for vector observations. Maintains hidden state across
    sequential calls; designed to plug into the network pool alongside MLP.
    ...
    Args:
        ...
        hidden_dim: dimension of the LSTM hidden state (default 128).
    ...
    Notes on shape conventions:
        ...
        - State convention: LSTM has an ``(h, c)`` tuple; GRU and RNN have ``h``
          only. Hidden state is initialized as zeros if state is None.
    """
    ...
    def initial_state(self, batch_size: int, device=None):
        """Return zero-initialized ``(h, c)`` hidden state for a batch."""
    ...
    def forward(self, obs, state=None):
        ...
        out, new_state = self.lstm(obs, state)  # out: (batch, seq, hidden)
