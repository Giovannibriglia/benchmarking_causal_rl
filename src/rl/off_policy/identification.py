...
class Grace:
    ...
    A declared-graph template CBN (two likelihood channels: observational and
    do(A)-mutilated by graph surgery), a null-calibrated regime ROUTER with
    channel-split detection statistics, and a bootstrap-ensemble interval —
    served per the B5 rule: no defect -> Q_obs; coverage defect -> Q_lower;
    confounding + healthy identification -> Q_do; confounding + unhealthy ->
    Q_lower. The estimator lives in ``src/rl/offline/grace/``; this strategy
    owns the three protocol hooks.

    ``critic_value`` routes to ``net.q_obs`` — a literal pass-through to the
    TRAINABLE observational head, so the base learner's training is exactly
    the observational floor's (the B6 reduction construction; routing happens
    only on the serving surface, ``GraceQNetwork.forward``).

    Five-keys invariant: ``requires_confounder_u = False`` — the CBN fit and
    the router consume only (S, A, R, S', done); the realized U is never read
    (leakage rule R5). ``needs_episode_grouping = True`` — the episode-latent
    EM and the router statistics need whole episodes.
    """

    requires_confounder_u = False  # five-keys: infers U, never reads it
    needs_episode_grouping = True  # episode-latent EM + router statistics

    def __init__(self, graph=None) -> None:
        self.graph = graph  # a CellGraph; bound by the builder when known
        self._machinery = None

    def bind(self, graph, machinery) -> None:
        """Builder handoff: the declared ``CellGraph`` (A9) and the fitted
        machinery (diagnostics source)."""
        self.graph = graph
        self._machinery = machinery

    def critic_value(self, net, x, batch):
        # Trainable observational head; identical training to the floor. The
        # routed estimand is the net's ``forward`` (serving surface), never a
        # learn-time hook.
        q_obs = getattr(net, "q_obs", None)
        return q_obs(x) if q_obs is not None else net(x)

    def graphical_precondition(self, graph=None) -> bool:
        """A9.3: REAL checks against the declared ``CellGraph`` — U -> A
        declared (else the confounded serving mode is structurally
        unjustified), proxies present/absent (governs proximal-style claims —
        currently never made), and an adjustment path for the declared
        structure. Reasons are computed by ``identification_report`` and
        logged by the machinery per cell."""
        g = graph if graph is not None else self.graph
        if g is None:
            return False
        from src.rl.offline.grace.cell_graph import identification_report

        rep = identification_report(g)
        return bool(rep["confounded_serving_ok"] and rep["adjustment_ok"])

    def statistical_diagnostic(self, batch_or_fit=None) -> dict:
        """The fitted machinery's telemetry (router components, ensemble
        width, separability, belief entropy) — logged, and consumed by the
        reporting layer's router-verdict columns."""
        if self._machinery is not None and self._machinery.ready:
            return dict(self._machinery.diagnostics())
        return {"grace_ready": 0.0}
