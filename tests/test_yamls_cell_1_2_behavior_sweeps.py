...
unaffected and lives in the baseline YAML. These tests check parse, the two
behavior fields, the off-policy-only algo list, and grid completeness. No run
...
def _check_behavior(behavior: str):
    for cell, stem, algo, is_masked in ARMS:
        for nnn, sval in STRENGTHS:
            ...
            # collection, so on-policy PPO is intentionally absent here.
