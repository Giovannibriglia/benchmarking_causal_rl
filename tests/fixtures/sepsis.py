...
from .causal_base import CausalEnv
from .legacy_cells import get_cell_config
...
class SepsisCausalEnv(CausalEnv):
    ...
    def do_reward(self, action: torch.Tensor) -> torch.Tensor:
        ...
        z = self._z
        p_u0 = torch.sigmoid(
            self.reward_logits[0, z, actions]
            - self.alpha * self.behavior_gate[z, actions]
        )
        p_u1 = torch.sigmoid(
            self.reward_logits[1, z, actions]
            + self.alpha * self.behavior_gate[z, actions]
        )
        p = 0.5 * (p_u0 + p_u1)
    ...
    def _factual_transition(self, actions: torch.Tensor) -> torch.Tensor:
        ...
        logits = (
            torch.log(base.clamp_min(1e-8)) + self.alpha * self._u.unsqueeze(-1) * conf
        )
    ...
    def do_transition(self, action: torch.Tensor) -> torch.Tensor:
        ...
        conf = self.transition_conf[z, actions]
        p0 = torch.softmax(
            torch.log(base_u0.clamp_min(1e-8)) - self.alpha * conf, dim=-1
        )
        p1 = torch.softmax(
            torch.log(base_u1.clamp_min(1e-8)) + self.alpha * conf, dim=-1
        )
        return 0.5 * (p0 + p1)
    ...
    def behavior_log_probs(self) -> torch.Tensor:
        ...
        if self.cell in (7, 8):
            logits = (
                logits
                + self.alpha * self._u.unsqueeze(-1) * self.behavior_gate[self._z]
            )
