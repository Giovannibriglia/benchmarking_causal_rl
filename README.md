...
# Causal Identifiability Cells

The repo implements the eight-cell experiments of *"Debugging Single-Agent
Reinforcement Learning through Simple Causal Lens"*: RL data-generating
regimes classified by (a) context Z observed/hidden, (b) online vs offline
with known/unknown behavior policy, (c) hidden confounder U entering both
action and reward. Headline metric: **normalized regret vs a Cell-1 PPO
reference**; secondary metric: the identifiability gap.

## Running

```bash
# online cells (1-2): plain benchmark mode on causal/ Gymnasium ids
python main.py --envs causal/cartpole-cell1 causal/cartpole-cell2 \
    causal/cartpole-cell2fs --algos ppo --deterministic

# offline cells (3-8): cell-spec YAMLs
python main.py --mode causal_cells --reproduce causal_cells/cell3_cartpole

# full discrete 5-seed sweep (hardened harness, validates + aggregates)
bash tools/run_matrix_sweep.sh

# figures + LaTeX tables (regenerate from CSVs alone)
python plot_causal.py --run-csvs runs/causal_cells_cell*/causal_cells_metrics.csv
```
...
| Piece | Where |
|---|---|
| Cell registry + causal/ env ids (masking, frame-stack variant) | `src/causal/cells.py`, `src/causal/masking.py` |
| Confounding axis: per-episode U, biased logging policy, dataset gate | `src/causal/confounding.py` |
| Offline data: Minari IO with logged propensities, dataset sources | `src/data/` |
| Offline algorithms: BC, d3rlpy CQL/IQL, ensemble-pessimistic (delphic-inspired), proximal, latent world model | `src/offline/` |
| OPE (naive/DM-FQE/IPW/DR), Kallus-Zhou intervals, oracle, regret protocol | `src/eval/` |
| Dataset collection (tiers, confounded grid) | `tools/collect_offline_tiers.py`, `tools/collect_confounded.py` |

Design notes, honest framings and paper-flagged findings live in
`docs/causal_cells.md`; per-cell expectations and gate decisions in
`docs/phase0_audit.md` and `docs/known_issues.md`.
